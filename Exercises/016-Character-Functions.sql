-- for more info https://docs.oracle.com/database/121/SQLRF/function002.htm#SQLRF20032
-- character functions
-- there are 2 types for character functions
-- case conversion functions and character manipulation

-- Ex1: case conversion function (upper, lower, initcap)

SELECT EMPLOYEE_ID, FIRST_NAME, upper(FIRST_NAME), lower(FIRST_NAME), initcap(first_name)
FROM EMPLOYEES;

-- please look to first_name: Jose Manule and see the initcap who it work
...
ORDER BY UPPER(FIRST_NAME); -- this example perferct whene creating search screens

-- Ex2: character manipulation functions
...
-- mis the start searching position, n the occourence
...
-- trim function
-- to understnd trim in vary good way, we will try to do examples using dual table
-- dual is a public table that you can use to view result from functions and calculation

SELECT * FROM DUAL;

-- so it is a table contain one column and only dummy value x
...
SELECT TRIM ( ' ' FROM ' Mohammad ALnajjar ') V FROM DUAL;

SELECT TRIM (LEADING ' ' FROM ' Mohammad ALnajjar ') V FROM DUAL;

SELECT TRIM (TRAILING ' ' FROM ' Mohammad ALnajjar ') V FROM DUAL;

SELECT TRIM (both ' ' FROM ' Mohammad ALnajjar ') V FROM DUAL;


SELECT TRIM ( 'M' FROM 'Mohammad ALnajjar') V FROM DUAL;

SELECT TRIM (LEADING 'M' FROM 'Mohammad ALnajjar') V FROM DUAL;

SELECT TRIM (TRAILING 'M' FROM 'Mohammad ALnajjar') V FROM DUAL;

SELECT TRIM ('  mohammad alnajjar  ') V FROM DUAL;
